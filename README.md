...
- iPhone 快捷指令入口：背部轻点后识别当前账单截图，预填金额、商家、日期、分类和账户，确认后再入账。
...
先保持电脑端使用 `--host` 启动，并确保手机和电脑连接同一个 Wi-Fi。然后在 iPhone“快捷指令”中新建快捷指令，依次添加：

1. “截屏”（搜索不到时搜索“屏幕截图”）。不要在前面添加“等待”，否则背部轻点后会产生明显停顿。
2. “从图像中提取文本”，输入选择上一步的屏幕截图。
3. “Base64 编码”，输入选择上一步提取的文本；点操作右侧的蓝色箭头展开选项，务必将“换行”设为“无”（不能使用默认的每 76 个字符换行）。
4. “文本”，内容填写以下地址，并在等号后插入上一步“Base64 编码”的结果：

```text
http://电脑局域网IP:5173/#/quick-entry?data=Base64编码后的文本
```

5. “打开 URL”，输入选择上一步的文本。

最后进入“设置 → 辅助功能 → 触控 → 轻点背面 → 轻点两下”，选择刚创建的快捷指令。使用时停留在微信或支付宝付款成功/账单详情页面，再轻点背面两下。识别结果只会预填，仍需用户确认后保存。

Base64 比“URL 编码”生成的地址短很多，可明显减少长截图文字被 iOS 截断或二次编码的概率。旧版 `?text=URL编码文本` 入口仍然兼容。

OCR 原文放在 URL 的 `#` 片段中，不会作为网络请求内容发送给网站服务器；应用读取后会立即从地址栏清除。使用局域网地址时，电脑必须保持项目运行；部署到 HTTPS 网站后可把快捷指令中的地址替换成正式地址。
